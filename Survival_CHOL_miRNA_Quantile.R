...
if (!dir.exists("Survival_Plot_miRNA_DEG")) dir.create("Survival_Plot_miRNA_DEG")   ### change directory name here

plot_surv <- function (clinical_patient, dataGE, Genelist, Survresult = FALSE, ThreshTop = 0.67, ThreshDown = 0.33, p.cut = 0.05) 
{
  ...
  for (i in 1:nrow(as.matrix(rownames(dataNormal)))) {
    ...
    if (!is.na(mRNAselected_values_ordered_top)) {
      ...
      if (Survresult == TRUE) {
        ...
        #print(tabSurv)
        file_name = paste0("Survival_Plot_miRNA_DEG/KM_Plot_", mRNAselected, ".pdf")
...
results <- plot_surv(clin, dataGE = rna, Genelist = common_names, ThreshTop = 0.6, ThreshDown = 0.4, Survresult = TRUE, p.cut = 0.05)
#results <- plot_surv(clin, dataGE = rna, Genelist = rownames(results), ThreshTop = 0.6, ThreshDown = 0.4, Survresult = TRUE, p.cut = 0.05)
results1 <- results[results$`Cancer Deaths with Top` >=3 & results$`Cancer Deaths with Down` >=3,]
unlink("Survival_Plot_miRNA_DEG/KM_Plot_*") 
results1.DEG <- plot_surv(clin, dataGE = rna, Genelist = rownames(results1), ThreshTop = 0.6, ThreshDown = 0.4, Survresult = TRUE, p.cut = 0.05)
...
save.image("Survival_CHOL_miRNA_Quantile.RData")
