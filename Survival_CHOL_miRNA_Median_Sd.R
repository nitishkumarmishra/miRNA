...
if (!dir.exists("Survival_Plot_miRNA")) dir.create("Survival_Plot_miRNA")   ### change directory name here
  ...
  for (i in 1:nrow(as.matrix(rownames(dataNormal)))) {
    ...
    if (!is.na(mRNAselected_values_ordered_top)) {
      ...
      if (Survresult == TRUE) {
        ...
        #print(tabSurv)
        file_name = paste0("Survival_Plot_miRNA/KM_Plot_", mRNAselected, ".pdf")
        dev.print(pdf, file_name, width = 8, height = 8)
        #sfit <- survfit(ttime ~ c(rep("low", nrow(cfu_onlyTOP)), rep("high", nrow(cfu_onlyDOWN))))
        #ggsurvplot(sfit, pval = TRUE, risk.table = "abs_pct", risk.table.height=0.15, risk.table.fontsize=4, surv.median.line = "hv", pval.size = 5,  main="Kaplan-Meier Overall Survival Curves", ylab = "Probability of survival", palette=c("blue4", "red4"))
        #dev.print(pdf, file_name, width = 10, height = 10)
      }
    }
  }
...
results <- plot_surv(clin, dataGE = rna, Genelist = common_names, Survresult = TRUE, p.cut = 0.05, Median = TRUE, Sdev = 0)
unlink("Survival_Plot_miRNA/KM_Plot_*") 
results1 <- plot_surv(clin, dataGE = rna, Genelist = rownames(results), Survresult = TRUE, p.cut = 0.05, Median = TRUE, Sdev = 0)
...
save.image("Survival_CHOL_miRNA_Median_Sd.RData")
